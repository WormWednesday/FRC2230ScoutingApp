...
import 'package:frc_scouting_app/views/big_screen_views/scouting_screen/matches/create_match.dart';
...
// TODO: need to post data to db when certain team wins, change teams in match info to be fks to teams table
...
class _ViewMatchesState extends State<ViewMatches> {
  ...
  Widget build(BuildContext context) {
    ...
    List<String> ids = matchesIds
        .toString()
        .substring(1, matchesIds.toString().length - 1)
        .replaceAll(RegExp(r'{match_id: '), '')
        .replaceAll('}', ' ')
        .replaceAll(' ', '')
        .split(
            ','); // FormatException happenns here bcs line converts empty list to int list OR its empty, in that case make a defualt case
    print(ids);
    print(matchInfo);
              ...
              children: [
                ExpansionPanelList(
                  ...
                  expansionCallback: (i, state) =>
                      setState(() => isOpen[i] = !state),
                  children: List.generate(matchInfo.length, (i) {
                    final List<String> matchInfoItemInList = matchInfo[i]
                        .toString()
                        .substring(1, matchInfo[i].toString().length - 1)
                        .replaceAll(RegExp(r' '), '')
                        .split(",");
                    // print(matchInfoItemInList); // DEBUG
                    // print('lol');
                    final List<String> matchInfoSorted = List.generate(
                        matchInfoItemInList.length,
                        ((i) => matchInfoItemInList[i].split(":")[1]));

                    return ExpansionPanel(
                        headerBuilder: (context, isExpanded) => head(
                            id: matchInfoSorted[
                                MatchInfoSortedIndexes.id.index],
                            idsList: ids,
                            competition: matchInfoSorted[
                                MatchInfoSortedIndexes.competition.index],
                            matchNumber: matchInfoSorted[
                                MatchInfoSortedIndexes.number.index],
                            matchType: matchInfoSorted[
                                MatchInfoSortedIndexes.type.index],
                            wonAlliance: matchInfoSorted[
                                MatchInfoSortedIndexes.wonAlliance.index]),
                        body: body(
                            idsList: ids,
                            teamNumbers: [
                              matchInfoSorted[MatchInfoSortedIndexes.r1.index],
                              matchInfoSorted[MatchInfoSortedIndexes.b1.index],
                              matchInfoSorted[MatchInfoSortedIndexes.r2.index],
                              matchInfoSorted[MatchInfoSortedIndexes.b2.index],
                              matchInfoSorted[MatchInfoSortedIndexes.r3.index],
                              matchInfoSorted[MatchInfoSortedIndexes.b3.index]
                            ],
                            matchId: matchInfoSorted[
                                MatchInfoSortedIndexes.id.index],
                            competition: matchInfoSorted[
                                MatchInfoSortedIndexes.competition.index],
                            matchNumber: matchInfoSorted[
    ...
    return SafeArea(
      ...
      Padding(
        ...
        child: Container(
          ...
          decoration:
              BoxDecoration(shape: BoxShape.circle, color: isMatchOverColor),
        ),
      ),
  ...
  body(
      ...
      required List<String>
          teamNumbers, // list format needs to be [r1, b1, r2, b2....]
          ...
          if (isMatchOver(matchId, idsList)) {
            ...
            return Column(
              ...
              children: [
                ElevatedButton(
                    onPressed: () =>
                        QuickTeamData(teamNum: "2230"), // TODO: needs to go to team match detail
                ...
                GridView.count(
                  ...
                  children: List.generate(
                      ...
                      (index) => ElevatedButton(
                            onPressed: () => QuickTeamData(teamNum: "2230"),
  ...
  Color getIsMatchOverColor(String id, List<String> ids) {
    ...
    */
    if (isMatchOver(id, ids)) {
      return widget.EMcolor;
    } else {
      return widget.OMcolor;
    }
  }
